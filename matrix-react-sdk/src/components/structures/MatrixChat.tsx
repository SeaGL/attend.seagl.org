...
export default class MatrixChat extends React.PureComponent<IProps, IState> {
        ...
        switch (payload.action) {
            ...
            case 'view_home_page':
                dis.dispatch({
                    action: 'view_room',
                    auto_join: true,
                    room_alias: "#SeaGL2021:seattlematrix.org"
                });
                break;
    ...
    private async onLoggedIn() {
        ...
        const autoJoinAliases = [
            ...
            "#SeaGL2021-Sessions-Upcoming:seattlematrix.org"
        ];
        const client = MatrixClientPeg.get();
        try {
    ...
    showScreen(screen: string, params?: {[key: string]: any}) {
        ...
        if (screen === 'register') {
            if (params?.hs === "ephemeral") {
                this.onServerConfigChange(serverConfigPresets["seagl.org"]);
            } else {
    ...
    onRegisterClick = (ephemeral: boolean = false) => {
        if (ephemeral) {
